...
sortButton.addEventListener("click", function () {
    ...
    sendMessageButton.addEventListener("click", function () {
        ...
        // }
        const city = prompt("Enter the city:"); // Prompt the admin to enter the city
        if (city) {
            sendMessageToUsers(message, city); //new thing
        }
    });
...
function sendMessageToUsers(message,city) {
    ...
    fetch('/users/sendMessage', {
        ...
        body: JSON.stringify({ message: message ,city: city})
    })
    .then(response => {
        if (response.ok) {
